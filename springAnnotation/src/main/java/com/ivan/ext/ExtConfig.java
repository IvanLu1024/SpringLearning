 ...
 *  BeanFactoryPostProcessor:beanFactory的后置处理器，在BeanFactory
 *  在标准初始化之后调用：所有的bean的定义已经保存加载到beanFactory，但是beanFactory的实例还没创建
 *
 ...
 *  * 		监听 ApplicationEvent 及其下面的子类（事件）；
 *  *
 *  * 	 步骤：
 ...
 *  *  1）、ContextRefreshedEvent事件（容器发布的事件）：
 *  *  	    1.容器创建对象：refresh()；
 ...
 *  *  				1、如果有Executor，可以支持使用Executor进行异步方式的派发；
 ...
 *  *
 *  *   SmartInitializingSingleton （接口）原理：->afterSingletonsInstantiated();
 *  *   		1）、ioc容器创建对象并refresh()；
 *  *   		2）、finishBeanFactoryInitialization(beanFactory);初始化剩下的单实例bean；
 *  *   			1）、先创建所有的单实例bean；利用for循环拿到所有需要创建的bean，getBean();
